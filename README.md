...
- **電文種別コード** `alert_type`（実機の受信情報種別シンボルに準拠）：
  `JALT`(国民保護情報) / `IFDA`(緊急連絡) / `EPRQ`(緊急地震速報) /
  `IOEQ`(地震情報・震度速報) / `ISSW`(津波情報) / `VOLC`(火山情報) / `WRMA`(気象情報)
- **情報種別** `alert_sub_type`：上記に対応する種別名（気象はさらに 気象警報・注意報／
  記録的短時間大雨情報／竜巻注意情報／土砂災害警戒情報／指定河川洪水予報 に細分）
- **優先度** `priority`：国民保護 15 ＞ 緊急地震速報 13 ＞ 津波 11 ＞ 火山 9 ＞
  緊急連絡 ＞ 震度速報 ＞ 地震・気象 0（複数同時受信時の主表示を決定）
- **表示対象 `allowed`**：緊急情報表示設定（＝外部IF動作ルール）の対象か

気象(WRMA)は JMA XML をパースし、`Body/Warning`（府県予報区）の `<Kind>`＋`Status`
（発表/継続/解除）から **特別警報 > 警報 > 注意報** を判定します。それ以外の情報種別は
種別ごとに表示レベルが決まります（国民保護＝最優先全画面、緊急地震速報/津波/火山/緊急連絡＝
全画面、地震情報/震度速報＝情報バナー）。
...
| 情報 | 注意報・地震情報・震度速報など | 待機画面＋下部に**右→左へ流れる**黄色テロップ |
| アラート | **警報級以上の表示対象** | 全画面 **赤**（種別・地域・本文・発表時刻、点滅） |
| アラート | **特別警報 / 国民保護** | 全画面 **紫** |

全画面アラートは右上の **「確認(ACK)」ボタン**（または `Enter`/`Space`）で確認でき、
待機画面へ復帰します（警報が継続中の間は上部に「確認済み（継続中）」リボンを表示、
「再表示」で戻せます）。新規・内容変化のアラートは再度全画面表示されます。

注意報・情報の発表時は、待機画面下部のバナーが**右から左へスクロール**（テロップ）します。
...
- **リアル**：**従来機の実画面（待機・アラート）をそのまま使用**。待機は実機の
  待機画像＋4情報ランプ点灯、アラートは情報種別ごとの実画面に発表内容を重畳。
  実画面は**細分**（津波＝大津波警報／津波警報／津波注意報／津波情報、火山＝噴火警報
  レベル別・噴火速報、国民保護＝弾道ミサイル等、地震＝緊急地震速報／地震情報）まで
  実機の画像選択ロジックに準拠。気象は実画像が無いため色面表示にフォールバック。
...
- **手動発報**（運用管理者以上）：情報種別＋**都道府県・市区町村**（実機マスタ準拠の
  47都道府県/1936市区町村）＋見出しを選んで画面に緊急情報を発報（試験・訓練用）。
  「全解除」で復帰。
- **緊急情報一覧**：全受信を時系列で既読/未読管理。**情報種別での絞り込み**・キーワード
  検索（地域/種別/本文）に対応。詳細では発表官署相当の見出し（JMA `Headline/Text`）・
  発表時刻（`ReportDateTime`）・発表中の種別・**XML原文**を確認。地震は震源・最大震度・
  マグニチュードを抽出。再送・同報・継続電文は内容同一で自動集約
...
- **Web 表示スタイル**：公開ページ（`/`）の待機スタイルをシンプル／パチモン／リアルから
  **デスクトップとは個別に**選択
...
配信します。
- `/` … **公開用 J-ALERT 表示画面**（待機 ↔ 全画面アラート、注意報テロップは右→左に流れる）。
  待機スタイル（シンプル／パチモン／リアル）は設定の「Web 表示スタイル」で切替。確認(ACK)ボタンあり。
- `/admin` … **管理用 緊急情報一覧**（既読/未読、ライト/ダーク、**スマホ対応**のレスポンシブ）
...
# 過去受信の生 JMA XML を再生（フォルダ＝全件、単一 .xml も可）
jalert-receiver --replay path/to/xml_archive/ --replay-interval 0
...
| `--replay PATH` | — | — | JSONL を再生、または生 JMA XML（フォルダ/単一ファイル）を再生 |
